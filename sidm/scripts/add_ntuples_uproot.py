...
https://github.com/CoffeaTeam/coffea-casa/issues/374 is resolved. This variant uses uproot
instead of PyROOT to read event counts and OriginalEventIndex.

Usage: python add_ntuples_uproot.py -o OUTPUT_CONFIG -n NTUPLE_NAME -c NTUPLE_COMMENT -d NTUPLE_ROOT_DIR
"""
...
parser.add_argument("--filelists-dir",
                    help=("Read '<sample>_skim.txt' or '<sample>_full.txt' fragments "
                          "from this directory instead of listing the remote directory"))
...
xrd_client = None if args.filelists_dir else client.FileSystem(redirector)
...
if args.filelists_dir:
    if args.samples is None:
        parser.error("--samples is required with --filelists-dir")

    list_kind = "skim" if args.skim else "full"
    samples = []
    for simple_name in args.samples:
        filelist = Path(args.filelists_dir) / f"{simple_name}_{list_kind}.txt"
        files = []
        sample_path = None
        for raw_line in filelist.read_text().splitlines():
            ...
            if line.startswith("- "):
                ...
                sample_path = line.split(":", 1)[1].strip()
        if sample_path is None:
            raise ValueError(f"No path entry in {filelist}")
        samples.append({
...
for sample in samples:
    if args.filelists_dir:
        simple_name = sample["simple_name"]
    ...
    print(f"{sample_name} --> {simple_name}")
    ...
    if args.skim:
        ...
        for file in files[:10000]:
            file_path = (
                f"{args.directory.rstrip('/')}/"
                f"{sample_path.strip('/')}/{file}"
            )
            try:
                with uproot.open(file_path) as in_file:
                    ...
                    tree = in_file["Events"]
                    file_skimmed_evts = tree.num_entries
                    if file_skimmed_evts == 0:
                        print(f"No events in {file_path}")
                        print("skipping")
                        continue

                    file_original_evts = int(
                        tree["OriginalEventIndex"].array(
                            entry_stop=1,
                            library="np",
                        )[0]
                    )
            ...
            if file_original_evts == 0:
                ...
                print(
                    skimmed_evts,
                    original_evts,
                    file_skimmed_evts/file_original_evts,
                    skimmed_evts/original_evts,
                )
